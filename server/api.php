...
try {
 ...
 if($path==='/api/businesses' && $method==='GET') {
  ...
  if(filter_var($_GET['offers']??false,FILTER_VALIDATE_BOOLEAN)) { $now=date('Y-m-d H:i:s'); $where.=" AND p.promotion_limit>0 AND EXISTS (SELECT 1 FROM promotions offer WHERE offer.business_id=b.id AND offer.starts_at<='$now' AND offer.expires_at>'$now')"; }
  if(filter_var($_GET['delivery']??false,FILTER_VALIDATE_BOOLEAN))$where.=' AND b.delivery_available=1';
  if(filter_var($_GET['home_service']??false,FILTER_VALIDATE_BOOLEAN))$where.=' AND b.home_service_available=1';
  if(filter_var($_GET['open_now']??false,FILTER_VALIDATE_BOOLEAN)){$day=['mon','tue','wed','thu','fri','sat','sun'][(int)date('N')-1];$time=date('H:i');$open=env('DB_DRIVER','sqlite')==='mysql'?"JSON_UNQUOTE(JSON_EXTRACT(b.hours_schedule,'$.{$day}.open'))":"json_extract(b.hours_schedule,'$.{$day}.open')";$close=env('DB_DRIVER','sqlite')==='mysql'?"JSON_UNQUOTE(JSON_EXTRACT(b.hours_schedule,'$.{$day}.close'))":"json_extract(b.hours_schedule,'$.{$day}.close')";$where.=" AND CASE WHEN JSON_VALID(b.hours_schedule) THEN ($open<='$time' AND $close>'$time') ELSE 0 END";}
