...
test('Localização pública na busca e perfil; promoções avançam a cada cinco segundos',async({page})=>{
 ...
 await page.clock.install();await page.clock.pauseAt(new Date());await page.goto('/');await expect(page.locator('.promo-slide')).toContainText('Oferta teste 1');await expect(page.locator('.map-link')).toHaveCount(1);await expect(page.locator('.map-link')).toHaveAttribute('href','https://maps.app.goo.gl/localTeste');await expect(page.locator('.map-link')).toHaveAttribute('target','_blank');
 ...
 await page.setViewportSize({width:390,height:844});expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBeTruthy();await page.screenshot({path:'test-results/improvements-mobile.png',fullPage:true});await page.locator('.card-title').first().click();await expect(page.getByRole('link',{name:'Abrir no Google Maps'})).toHaveAttribute('href','https://maps.app.goo.gl/localTeste');
});
