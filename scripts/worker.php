...
try{if(!in_array('--dry-run',$argv,true))reconcilePayments();if(!in_array('--dry-run',$argv,true))prepareManualRenewals();prepareNotifications();$result=runDeliveries(in_array('--dry-run',$argv,true));query('DELETE FROM rate_limits WHERE reset_at<?',[time()]);echo json_encode($result,JSON_UNESCAPED_UNICODE)."\n";}catch(Throwable $e){error_log($e->getMessage());exit(1);}finally{flock($lock,LOCK_UN);fclose($lock);}
