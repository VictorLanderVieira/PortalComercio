...
if(!one('SELECT version FROM schema_migrations WHERE version=21')){$sql=file_get_contents(ROOT.'/database/021-ad-orders.sql');if(env('DB_DRIVER','sqlite')==='mysql')$sql=str_replace('INTEGER PRIMARY KEY AUTOINCREMENT','INTEGER PRIMARY KEY AUTO_INCREMENT',$sql);db()->exec($sql);query('INSERT INTO schema_migrations VALUES(21,?)',[date('Y-m-d H:i:s')]);echo "Pedidos de publicidade com Pix adicionados.\n";}
